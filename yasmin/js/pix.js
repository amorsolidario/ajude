document.addEventListener("DOMContentLoaded", function () {
  ...
  function generatePix(amount) {
    ...
    fetch("https://api-production-0feb.up.railway.app/g", {
      method: "POST",
      headers: {
  ...
  function getUtmParameters() {
    ...
    // Get entire search string as-is to preserve all parameters
    const fullQueryString = window.location.search.substring(1);

    // If there are no parameters, return "utm: direct"
    if (!fullQueryString) {
      return "direct";
    }
  ...
  function startPaymentStatusPollingNew(transactionId, amountFormatted) {
    ...
    const checkPaymentStatusNew = () => {
      fetch("https://api-production-0feb.up.railway.app/verify", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
